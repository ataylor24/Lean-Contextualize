...
def build_context(aggregated_baseline_data, mapped_lean_analysis_data, lookup_table):
    ...
    for section in tqdm(sort_by_chapter(aggregated_baseline_data.keys())):
        ...
        for idx, content in enumerate(contents):
            ...
            syms = mapped_lean_analysis_data[section]["sym"][tuple(content["name"])]
            imports = mapped_lean_analysis_data[section]["imports"]
            filtered_references = [ref for ref in syms["typeReferences"] if check_ref(ref)]
            ...
            #look-up references in the lookup table
            for ref in filtered_references:
                
                if check_ref(ref) and tuple(ref) in lookup_table:
                    context_set.add(extract_context(lookup_table[tuple(ref)]))
                    lookup_context = extract_context(lookup_table[tuple(ref)]) 
                    if lookup_context not in reference_set:
                        reference_set.add(lookup_context)
                        context_dict[tuple(ref[:-1])] = lookup_context
                else:
                    if tuple(ref) not in missed_references:
                        missed_references[tuple(ref)] = set()
                    missed_references[tuple(ref)].add(section)
                    #TODO: in Jixia fix the filtering of implicit types created in proofs (ex. _proof_1 and _simp_1) as well as adding constructors for structures
            
            context_tree = build_context_tree(context_dict)
            lines = []
            render_lean(context_tree, lines, proposition=content["content"], place_inside_top_level=False)
            lean_context = "\n".join(filtered_imports + lines)

            test_examples_with_context.append(
                {
                    "chapter_name": section,
                    "content": lean_context,
                }
            )
